...
| `BILI_CODEX_COMPACT` | Codex native-compaction handling: `pass` (default) forwards codex's compaction requests upstream (native compaction backstops); `intercept` forges a local handoff to the ACP state when the safety gate passes (transform ok + steady-state usage < 90% of the window + at least one active compressed block) — trigger form forges a 2-frame SSE, endpoint form forges `{output}` — and never contacts upstream. Forged ACP summaries are re-injected as a developer message on every subsequent turn so compressed content stays visible after codex truncates its history. On any gate failure the request passes through untouched. |
