...
| `BILI_RLM_INHERIT` | Set `0` to disable derived-session inheritance (#1333). When a plugin conversation explicitly declares a parent (`x-bili-plugin-parent-conversation` header, or `parentId` in the `POST /__bili/plugin/register` body) — e.g. Prime's RLM inline child sessions — its first request seeds it with a **dormant** copy of the parent's compression archive, so the child can immediately `search_context` / `decompress` everything the parent had compressed. Seeded blocks are dormant: invisible in rebuilt history, inert for nudge decisions, exempt from orphan-GC; decompression is self-contained (originals travel with the blocks, so it keeps working after the parent session is evicted or deleted). Default is ON. `"rlmInherit": false` in the config file does the same; the env var wins. Distinct from `BILI_FORK_ADOPTION` (#629): that one infers anonymous content forks, this one covers explicitly host-declared parent-child relationships. |
