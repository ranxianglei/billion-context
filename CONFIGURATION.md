...
- **Default:** `default` — except when `promptPack` is **unset at every level**, in which case the sticky lean canary below may assign `lean` per session
- **Status:** ACTIVE
- **Description:** Select a named prompt pack — a curated surface preset covering tool descriptions, compress system-prompt sections, and nudge sections — resolved from the kernel's pack chain: **project** `./.billion-context/packs/<name>.json` → **user** `<configDir>/packs/<name>.json` → **builtin** (`default`, `lean`). Built-in `lean` swaps the four ACP tool descriptions for one-liners (no snippet/guideline chrome); on the pi host it additionally ships condensed system-prompt sections (compact acp-tags/summary-context guidance and a shorter how-to-compress) via the pack's pi adapter — the proxy-side compression rules stay default. Unknown names fall back to the identity surface with a one-time warning. Same three-level merge as the other fields; pack-surface sections (tool/section overrides) apply directly, without the `acknowledgePromptsRisk` gate — that gate governs only inline `compress.prompts` rule-text overrides. Note a pack's `prompts` block is ignored by this proxy: rule-text overrides are possible only via inline `compress.prompts`. Requires `acp-kernel` >= 0.0.66.

**Sticky lean canary (#1408).** When `compress.promptPack` is unset at every level (global, provider/route, model), each session is assigned a builtin pack **once** and keeps it for its whole life: a deterministic hash of the session id (`sha256(sessionId)` bucket `% 100`) is compared against [`BILI_PROMPT_PACK_CANARY_PCT`](#environment-variables) (default `10` — roughly 10% of sessions get `lean`, the rest `default`). The assignment is stamped onto the session (`meta.packCanary`, persisted with it), so it survives restarts and proxy-instance moves and **never re-rolls when the knob changes**: raising the pct admits not-yet-assigned sessions at their own hash; setting it to `0` stops NEW lean assignments without flipping any already-assigned session mid-life. Any explicit `promptPack` — including an explicit `"default"` — always wins and is never canaried. One info log line fires at first assignment (`prompt-pack canary: lean (pct=10, session=…)`), and the `/acp` panel's active-pack line shows the canaried value like any explicit setting.
...
| `BILI_CHAIN_CONTENT` | Set `0` to turn OFF the ACP-artifact content detection of bili→bili chain awareness (#1086, advisory-only since #1357): when an inbound request carries compression artifacts (render tags / historical `acp_status`+`search_context` tool calls) but neither the `x-bili-hop` header nor local compression state for its session, bili records ONE advisory observation (a single `[chain]` warn, visible in `/acp`) and processes the request normally — the session establishes its own state. Historical ACP-shaped content is user-authored text (AGENTS.md examples, docs, pastes) far more often than a stripped-header relay, so it is no longer decisive; only the `x-bili-hop` header still forces byte-identical passthrough. Trade-off: a bili→bili relay that strips `x-bili-hop` may now double-process until signed request-bound chain proof lands (#1357 Phase 2/3). Set `0` to disable the detection itself (no observation, no warn). Default is ON. `"chainContentDetection": false` in the config file does the same; the env var wins. The `x-bili-hop` signal itself is unaffected by this switch. |
...
| `BILI_PROMPT_PACK_CANARY_PCT` | Sticky lean prompt-pack canary percentage (#1408): integer `0`–`100`, default `10`; non-numeric or out-of-range values fall back to `10`. Only sessions whose `compress.promptPack` is unset at every level are eligible — any explicit setting (including `"default"`) wins and is never canaried. Each session's outcome is assigned once (deterministic session-id hash vs the current pct) and persisted with the session, so moving the knob never re-rolls an existing session: `0` stops new lean assignments, `100` assigns lean to every not-yet-assigned session. See [`promptPack`](#promptpack). |
