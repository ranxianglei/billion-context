...
- `x-bili-plugin-parent-conversation: <parent-conversation-id>` (optional) — declares that this conversation was **derived from** another conversation of the same agent (e.g. Prime's RLM inline spawn — pi writes `parentSession` into the child session file's header). See *Derived sessions* below. Lanes that bind through the identity register instead of per-request headers (omp) carry the same declaration as an optional `parentId` field in the `POST /__bili/plugin/register` body.
...
**Derived sessions (#1333).** When a fresh plugin session declares a parent (header above, or `parentId` in the register body), its first request seeds the session with a **dormant copy of the parent's compression archive**: every inherited block plus its captured originals, message-ref entries, and id-space continuation. The child can then `search_context` across the whole archive (one global scoring pass alongside its own blocks) and `decompress` any inherited block — self-contained, because the originals travel with the blocks, so it keeps working after the parent session is evicted or deleted. Inherited blocks stay **dormant**: invisible in rebuilt history, inert for nudge/summarization decisions, exempt from orphan-GC, and they never re-enter the wire. Access is one hop per declaration — a grandchild reaches great-grandparent content because each generation carries its ancestors' blocks forward. The header is honored only on plugin-bound requests (a plain client cannot seed its session from someone else's archive); seeding is controlled by `rlmInherit` / `BILI_RLM_INHERIT` (default ON). This is distinct from fork adoption (`BILI_FORK_ADOPTION`, #629), which infers anonymous content forks — here the relationship is explicitly declared by the host.
