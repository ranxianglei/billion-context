...
test("export of an uncompressed session states honestly that content is unavailable", () => {
    for (const full of [false, true]) {
        const md = renderHandoff(makeSession("abc123", "my uncompressed chat"), full);
        assert.match(md, /No active compression blocks\. Original messages are only persisted when they are compressed into a block/);
