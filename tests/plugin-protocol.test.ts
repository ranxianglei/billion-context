...
test("plugin manifest serves the exact wire tool schemas, headers and version", async () => {
    ...
    try {
        ...
        assert.deepEqual([...manifest.toolNames].sort(), ["absorb", "acp_rule", "acp_status", "compress", "decompress", "search_context"]);
        const names = manifest.tools.anthropic!.map((t) => t.name).sort();
        assert.deepEqual(names, ["absorb", "acp_rule", "acp_status", "compress", "decompress", "search_context"]);
