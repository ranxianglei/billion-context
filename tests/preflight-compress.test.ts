...
test("e2e: model switch to a smaller window → preflight compresses before forward", async () => {
    ...
    const proxy = await startServer({
        ...
        // CCR is default-ON in proxy mode since #1425 — these rigs pin
        // ccr.enabled=false explicitly so the exact-budget boundaries below
        // stay calibrated WITHOUT the acp_retrieve schema (~0.2k). If the
        // boundary shifts anyway, someone changed the schema/estimate math —
        // treat that as a regression tripwire. CCR-on interplay with the
        // cascade is covered by e2e-compress-cascade.test.ts; default-on
        // itself is pinned by ccr-v2 + wire-contract lanes.
