...
test("wire-contract C: default manifest serves only legal base tools", () => {
    ...
    assert.ok(anthNames.has("acp_rule"), "default manifest: acp_rule advertised — on by default since #1399");
...
const BASE_EXPECTED: Record<Wire, string[]> = {
    anthropic: [...BILI_ACP_TOOLS_ANTHROPIC.map((t) => t.name), "acp_rule"],
    "openai-chat": [...BILI_ACP_TOOLS_OPENAI.map((t) => t.function.name), "acp_rule"],
    responses: [...BILI_ACP_TOOLS_RESPONSES.map((t) => t.name), "acp_rule"],
    google: [...BILI_ACP_TOOLS_GOOGLE.map((t) => t.name), "acp_rule"],
};
...
test("wire-contract D: opt-in lane (absorb+rules+ccr) forwards the extended surface legally", async () => {
    ...
    try {
        ...
        const fwdNames = extractForwardedNames(lane.wire, fake.requests[0].body);
        const expected = [...BASE_EXPECTED[lane.wire], ...lane.clientToolNames, "absorb", "acp_retrieve"].sort();
        assert.deepEqual(fwdNames.sort(), expected, "opt-in lane forwards base (incl. default-on acp_rule, #1399) + absorb/acp_retrieve + client tools");
