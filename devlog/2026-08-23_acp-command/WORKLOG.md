...
### 8.4 Wire-level tool rename to bili_* (avoid name clash with billion-context-pi)

- **Request**: user asked to rename the proxy's 4 ACP tools to `bili_*` so a user
  who has BOTH billion-context and billion-context-pi installed doesn't see two
  sets of identically-named tools. Also rename the `/acp` command to `/bili`.
- **Design**: wire-level rename in the proxy (NOT in acp-kernel, which would
  affect billion-context-pi). The kernel's prompt text is parameterized
  separately via `withToolNames` (acp-kernel PR #138).
- **Changes**:
  - `src/compress-tool.ts`: added `BILI_TOOL_NAMES` (compress→bili_compress,
    decompress→bili_decompress, search_context→bili_search_context,
    acp_status→bili_status), `BILI_TOOL_ALIASES` (reverse), `biliToolName()`
    (maps bili_* → canonical), `renameTools`/`renameOpenaiTools` (generic,
    preserve type), `BILI_ACP_TOOLS_{ANTHROPIC,OPENAI,RESPONSES}` (renamed
    tool definitions), `BILI_PROXY_TOOL_NAMES` (detection set: canonical +
    bili_*).
  - `src/server.ts`: injection sites use `BILI_ACP_TOOLS_*` (the model sees
    bili_* names).
  - `src/loop/core.ts`: `executeProxyTool` maps bili_* → canonical at the top;
    detection uses `BILI_PROXY_TOOL_NAMES`.
  - `src/loop/adapter-responses.ts`, `src/compress-loop-responses.ts`,
    `src/plugin.ts`: detection uses `BILI_PROXY_TOOL_NAMES`.
  - `src/stream-openai.ts`, `src/stream-responses.ts`: COMPRESS_TOOL_NAME
    checks also match `BILI_TOOL_NAMES.compress`.
  - `src/agent/pi.ts`: `/acp` → `/bili`.
  - `src/plugin.ts`: manifest `toolNames` lists only bili_* names (what the
    plugin registers natively); tool definitions use `BILI_ACP_TOOLS_*`.
- **Tests**: updated 6 test files (plugin-protocol, plugin-agent,
  launcher-plugin-mode, codex-official, e2e-anthropic, e2e-export-handoff,
  e2e-responses-chat-relay) to expect the bili_* names.
- **Verification**: typecheck PASS, 526 tests pass, build PASS.
