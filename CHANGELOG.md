...
- **omp chat-completions providers: `/acp` stuck on "No model request in this conversation yet" + wire mode never upgrading to native**: omp's chat-completions payloads carry no conversation marker at all (no `prompt_cache_key` / `session` / `user` field — verified on omp 17.3.8), so the proxy keyed the session on a hash of the first user message while the omp plugin's identity register (and the `/acp` lookup) use the omp session id — they could never match. Long sessions silently kept working in wire mode, but `/acp` showed the armed-but-idle message forever and native tools never bound. The omp plugin now stamps `prompt_cache_key` (the omp session id) onto chat-shaped provider payloads via the `before_provider_request` payload rewrite (responses payloads already carry it natively; anthropic-wire and already-keyed payloads untouched), and the proxy's OpenAI path honors `prompt_cache_key` for identity, affinity and plugin-conversation recording — exactly like the Responses path. Verified end-to-end against a chat mock: panel resolves, `pluginAgent: "omp"` binds, wire injection suppressed.
