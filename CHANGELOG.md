...
- **Windows: a space in the profile no longer truncates the launched client or its `-e` extension path (#679)**: `runClient` spawned with `shell: true` and an unquoted command, so cmd.exe re-split the line on whitespace — any Windows user whose profile contains a space (which is exactly where `omp.exe` and the npm-global billion-context package live) got `'C:\Users\John' is not recognized as an internal or external command`. Quoting only the command repairs the crash and leaves the worse half: the args stay truncated, `-e` points at a path cut at the first space, the extension never loads, and compression silently never activates — the "92% context with zero compressions" class, with no error shown. The shell is now used only where it is genuinely required (a `.cmd`/`.bat` npm shim) and that case gets an explicitly quoted line passed verbatim (`cmd.exe /d /s /c`, `windowsVerbatimArguments`); a real `.exe`, node, or an extensionless binary spawns directly, where the OS quotes both the executable path and argv, spaces included. Tokens without whitespace stay bare, so a space-free launch is byte-identical to before, and the shell path's per-launch `DEP0190` warning is gone.
