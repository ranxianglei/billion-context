...
- **acp-loop auto-retry on no-content upstream truncation (issue #221)**: a relay cutting a long-running request (e.g. MiniMax-M3 thinking server-side for minutes) before any token is emitted ends the upstream SSE with no completion event and zero content. PR #239 made that visible (`[acp-proxy: upstream stream truncated ...]`) but the agent still stopped, because the proxy surfaced the error and terminated the turn. A truncation that produced NO text/reasoning is now a clean re-send — nothing reached the client, so the loop re-requests the same body and continues (the retry is treated as a later round, so `message_start` is not re-emitted and no prefix is duplicated). Bounded by `BILI_TRUNCATION_RETRY_MAX` (default 2 retries; `0` restores the fail-fast error). Deliberately NOT retried — the visible error still surfaces — when: a partial content was already streamed (the model would regenerate from scratch and duplicate the already-streamed prefix); a real tool call was already flushed to the client (the client will execute it and follow up, and a re-sent response would land behind that tool call in the same stream); or the truncation hits the final loop round (surfacing the error beats a degenerate empty completion).
