...
- **`bili omp` zero-config native plugin (`-e` injection)**: omp does NOT ship the bili plugin — the native `/acp` experience previously depended on `bili plugin install omp` having been run (and silently degraded when the config entry was later removed, which is how a long omp session ended up at 92% context with zero compressions). The omp launcher branch now mirrors pi: when the config carries no loadable bili entry it prepends `-e dist/agent/omp.js` (loads for this run only, writes nothing); a loadable entry — including one pointing at an older install — is left alone so nothing double-loads. Stale entries whose target file no longer exists do not suppress the injection. Note: omp (unlike pi) excludes extension-registered tools from its model-facing tool surface (verified on omp 17.3.8 — only its internal title request sees them), and its fork emits no `before_provider_headers`, so omp keeps wire-injected tools + `prompt_cache_key` identity; the injected plugin's value there is the native `/acp` command.
...
- **Relay-reported usage floored at the kernel estimate (issue #256)**: some relays report `input_tokens` far below the real context (aihub/MiniMax-M3 reported 5720 while the kernel counted 29238 pending T1 alone; a later turn reported 1877), so the nudge keyed on that number never fired and compression never happened proactively — the context grew unbounded until the relay itself rejected the request. Every usage recorder (wire loop, plugin passthrough, non-streaming JSON) now floors the reported context size at the kernel's own estimate (sum of pending T1+T2+T3) when the report is materially below it (>10% AND ≥2048 tokens); a 0 report (never legitimate for a non-empty prompt) is replaced by a significant estimate. The floor is conservative — the estimate is a lower bound of the true context, so it can only ever make the nudge slightly more eager, never less — and each engagement logs a clear `[acp-usage] ... under-reported usage` line so a broken relay is visible in the log. Cumulative `inputTokens` billing stats stay raw.
