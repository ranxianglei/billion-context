...
- **Codex Responses session continuity**: ChatGPT account identity now survives OAuth bearer rotation; cross-checked per-agent `thread-id` metadata replaces the legacy `instructions`-based namespace guess; and terminal `compaction_trigger` requests stay in the base conversation and bypass ACP injection, preserving the upstream requirement that the trigger remain the final input item. ACP state is rebased only after native compaction succeeds.
