...
| WC-4 | Tool-call `arguments` are user intent (#1039 invariant) and must survive the round trip byte-semantically. | Unwrap: `arguments` string → JSON parse → `input` object (malformed → `{}`). Rewrap: `input` object → `JSON.stringify` → `arguments`. bili's own proxy tools (`compress`, `decompress`, `search_context`, `acp_status`, `bili_*`) are ephemeral in proxy mode — executed server-side, NEVER forwarded to the client; real tool calls are replayed verbatim from the upstream's original event line. |
