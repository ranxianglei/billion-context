...
//
// The OpenCode 2.0 (V2) half is shared with the no-launcher npm-plugin entry
// (#809) in src/agent/opencode-v2.ts; only the launcher-specific lazy proxy
// detection is wired here.

import { fetchProxyVersion, proxyBaseFromEnv, proxyBaseFromUrl } from "./shared.js";
...
const setup = createOpencodeV2Setup({
    lazyResolveProxyBase: (url) => proxyBaseFromUrl(url) ?? proxyBaseFromEnv(),
});
