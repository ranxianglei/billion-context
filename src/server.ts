...
async function preflightCompressIfNeeded(
    ...
    // guaranteed-400 forward into a false "fits".
    if (result.compressedRanges > 0) {
        ...
        if (estimateCoreMessages(rebuilt.processedMessages) < limit) return rebuilt;
    } else if (estimateCoreMessages(prepared.processedMessages) < limit) {
