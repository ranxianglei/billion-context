...
| `BILI_RLM_INHERIT` | 设为 `0` 关闭派生会话继承（#1333）。当插件会话显式声明了父会话（`x-bili-plugin-parent-conversation` 头，或 `POST /__bili/plugin/register` 请求体中的 `parentId`）——如 Prime 的 RLM inline 子会话——其首个请求会用父会话压缩档案的**休眠副本**初始化该会话，子会话立即可对父会话已压缩的全部内容执行 `search_context` / `decompress`。继承块处于休眠状态：不出现在重建的历史中、不参与 nudge 决策、不受孤儿 GC 回收；解压自包含（原始内容随块复制，父会话被逐出或删除后依然可用）。默认开启。配置文件中设 `"rlmInherit": false` 效果相同；环境变量优先。与 `BILI_FORK_ADOPTION`（#629）不同：后者推断匿名的内容分叉，本项处理宿主显式声明的父子关系。 |
